...
const getWorkshopReviews = async (workshopId, page, limit, sort) => {
    // Verify workshop exists
...
const getWorkshopReviewStats = async (workshopId) => {
    const workshop = await WorkShop.findById(workshopId);
    if (!workshop) {
        throw new AppError(StatusCodes.NOT_FOUND, "Workshop not found");
    }
...
const updateReview = async (reviewId, payload, userId, userRole) => {
...
const getUserReviewForWorkshop = async (workshopId, userId) => {
