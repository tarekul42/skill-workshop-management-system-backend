...
const getAuditLogs = async (params) => {
    const { page = 1, limit = 20, collectionName, action, performedBy, documentId, startDate, endDate, } = params;
    const filter = {};
    const collectionNameFilter = toEqFilter(collectionName);
    if (collectionNameFilter)
        filter.collectionName = collectionNameFilter;
    ...
    if (performedByFilter)
        filter.performedBy = performedByFilter;
    const documentIdFilter = toEqFilter(documentId);
    if (documentIdFilter)
        filter.documentId = documentIdFilter;
    if (startDate || endDate) {
        if ((startDate && typeof startDate !== "string") ||
            (endDate && typeof endDate !== "string")) {
            throw new AppError(StatusCodes.BAD_REQUEST, "Invalid date range query parameter type");
        }
        const createdAtFilter = {};
        if (startDate) {
            const gteDate = new Date(startDate);
            if (Number.isNaN(gteDate.getTime())) {
                throw new AppError(StatusCodes.BAD_REQUEST, "Invalid startDate query parameter");
            }
        ...
        if (endDate) {
            ...
            if (Number.isNaN(lteDate.getTime())) {
                throw new AppError(StatusCodes.BAD_REQUEST, "Invalid endDate query parameter");
            }
