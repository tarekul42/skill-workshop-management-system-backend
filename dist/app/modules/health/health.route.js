 ...
 *         $ref: "#/components/responses/InternalServerError"
 */
router.get("/check-version", (_req, res) => {
    res.json({
        version: "1.0.0",
 ...
 *         $ref: "#/components/responses/InternalServerError"
 */
router.get("/dashboard", checkAuth(UserRole.ADMIN, UserRole.SUPER_ADMIN), async (_req, res) => {
    const startedAt = Date.now();
    let redisMemoryBytes = null;
    ...
    const overallStatus = redisMemoryBytes !== null && dbLatencyMs !== null && queueLength !== null
        ? "healthy"
        : "degraded";
    res.status(200).json({
