...
const deleteLevel = async (id) => {
    ...
    if (linkedWorkshops > 0) {
        throw new AppError(StatusCodes.CONFLICT, `Cannot delete level because it is referenced by ${linkedWorkshops} workshop(s)`);
    }
    await auditLogger({
...
const getSingleWorkshop = async (slugOrId) => {
    const query = mongoose.Types.ObjectId.isValid(slugOrId)
        ? { $or: [{ _id: slugOrId }, { slug: slugOrId }] }
        : { slug: slugOrId };
    const workshop = await WorkShop.findOne(query).populate("createdBy", "name email picture expertise bio");
    if (!workshop) {
...
const updateWorkshop = async (id, payload, currentUser) => {
...
const deleteWorkshop = async (id, currentUser) => {
    const existingWorkshop = await WorkShop.findById(id);
    if (!existingWorkshop) {
        throw new AppError(StatusCodes.NOT_FOUND, "Workshop not found");
    }
    ...
    const isOwner = existingWorkshop.createdBy?.toString() === currentUser.userId;
    if (!isAdmin && !isOwner) {
        throw new AppError(StatusCodes.FORBIDDEN, "You are not authorized to delete this workshop");
    }
    await auditLogger({
        action: AuditAction.DELETE,
        collectionName: "WorkShop",
