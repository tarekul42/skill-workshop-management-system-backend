...
const verifyOtp = async (email, otp) => {
    ...
    if (savedOtp !== hashOtp(otp)) {
        ...
        if (attempts > 5) {
            ...
            throw new AppError(StatusCodes.TOO_MANY_REQUESTS, "Too many failed attempts. Please request a new OTP.");
        }
        throw new AppError(StatusCodes.UNAUTHORIZED, "Invalid OTP");
    }
    // OTP is correct — mark user as verified and clean up
    await Promise.all([
        User.updateOne({ email: { $eq: normalizedEmail } }, { isVerified: true }, { runValidators: true }),
