 ...
 *         $ref: "#/components/responses/InternalServerError"
 */
router.post("/forgot-password", authLimiter, validateRequest(forgotPasswordZodSchema), AuthControllers.forgotPassword);
/**
 * @openapi
 * /auth/reset-password:
 *   post:
 ...
 *         $ref: "#/components/responses/InternalServerError"
 */
router.post("/reset-password", authLimiter, checkResetToken, validateRequest(resetPasswordZodSchema), AuthControllers.resetPassword);
/**
 * @openapi
 * /auth/google:
 *   get:
...
router.get("/google/callback", authLimiter, async (req, res, next) => {
    ...
    if (!queryState) {
        return res.redirect(`${envVariables.FRONTEND_URL}/login?error=${encodeURIComponent("Missing OAuth state parameter")}`);
    }
    try {
    ...
    passport.authenticate("google", (err, user, info) => {
        ...
        if (!user) {
            ...
            return res.redirect(`${envVariables.FRONTEND_URL}/login?error=${encodeURIComponent(reason)}`);
        }
        req.user = user;
        next();
    })(req, res, next);
